...
import community.flock.wirespec.showcase.movemoney.payments.api.MonetaryValue
import community.flock.wirespec.showcase.movemoney.payments.api.Payment
import community.flock.wirespec.showcase.movemoney.payments.api.PaymentInput
import community.flock.wirespec.showcase.movemoney.payments.api.PostPaymentEndpoint
import community.flock.wirespec.showcase.movemoney.payments.api.validate
import org.springframework.stereotype.Repository
import java.time.Instant
import community.flock.wirespec.showcase.movemoney.payments.api.Currency as CurrencyDto
        ...
        paymentsClient
            .getPayments(
                ...
                when (res) {
                    ...
                    is GetPaymentsEndpoint.Response400 -> throw RuntimeException(
                        "Something went wrong getting payments for ${userAccountInfo.accountNumber}. Details: ${res.body}",
                    )
                    is GetPaymentsEndpoint.Response500 -> throw RuntimeException(
                        "Something went wrong getting payments for ${userAccountInfo.accountNumber}. Details: ${res.body}",
                    )
                }
            }
        ...
        paymentsClient
            .getPayment(
                ...
                when (res) {
                    ...
                    is GetPaymentByIdEndpoint.Response400 -> throw RuntimeException(
                        "Something went wrong getting payment with id $transactionId for userId ${user.id}. Details: ${res.body}",
                    )
                    is GetPaymentByIdEndpoint.Response500 -> throw RuntimeException(
                        "Something went wrong getting payment with id $transactionId for userId ${user.id}. Details: ${res.body}",
                    )
                    is GetPaymentByIdEndpoint.Response404 -> throw ResourceNotFoundException(
                        "Transaction with id $transactionId was not found. Details: ${res.body}",
                    )
                }
            }
        ...
        return paymentsClient
            .postPayment(
                ...
                when (res) {
                    ...
                    is PostPaymentEndpoint.Response400 -> throw RuntimeException(
                        "Something went wrong creating a payments from ${paymentInput.senderAccountNumber} to ${paymentInput.recipientAccountNumber}. Details: ${res.body}",
                    )
                    is PostPaymentEndpoint.Response500 -> throw RuntimeException(
                        "Something went wrong creating a payments from ${paymentInput.senderAccountNumber} to ${paymentInput.recipientAccountNumber}. Details: ${res.body}",
                    )
                }
            }
...
private fun Payment.internalize(): Transaction =
    Transaction(
        transactionId = id,
        timing =
            Transaction.Timing(
                Instant.ofEpochMilli(transactionCreatedTimestamp),
                transactionCompletedTimestamp?.let(Instant::ofEpochMilli),
            ),
...
private fun CurrencyDto.internalize(): Currency =
    also {
        if (!it.validate()) {
